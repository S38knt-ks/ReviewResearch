...
from analyze_dependency import DependencyAnalyzer, PhraseContent, LinkProp, REQUIREMENT_POS_LIST
...
def main(args):
    ...
    review_jsons = [pathlib.Path(f).resolve() for f in glob_recursively('{}/**'.format(review_dir))
    ...
    for json_path in tqdm(review_jsons, ascii=True):
        for option in option_list:
            ...
            review_data = json.load(json_path.open(mode='r', encoding='utf-8'), object_pairs_hook=OrderedDict)
            ...
            json.dump(out_data, out_file.open(mode='w', encoding='utf-8'), ensure_ascii=False, indent=4)
