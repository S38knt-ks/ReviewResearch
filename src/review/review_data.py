...
class ReviewPageJSON:
    ...
    def build(self):
        ...
        property_attrs = {property_name: obj for property_name, _ in inspect.getmembers(ReviewPageJSON, lambda o: isinstance(o, property))
                                             for name, obj in self.__dict__.items() if name.endswith(property_name)}
    ...
    def load(cls, json_path):
        ...
        review_page = json.load(pathlib.Path(json_path).open(mode='r', encoding=cls.codec),
