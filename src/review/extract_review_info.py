...
from review.review_data import ReviewPageJSON, ReviewInfo, StarsDistribution
...
class ReviewInfoExtractor:
    ...
    def _extract_landmarks(self, html):
        ...
        landmarks_div = bs.find(PRODUCT_DIV_FINDATTRS.name, PRODUCT_DIV_FINDATTRS.attrs)

        maker = landmarks_div.find(MAKER_FINDATTRS.name, MAKER_FINDATTRS.attrs).text.strip()

        ave_star = self._extract_stars(landmarks_div.find(AVERAGE_STAR_FINDATTRS.name,
                                                          AVERAGE_STAR_FINDATTRS.attrs))

        total_reviews = int(landmarks_div.find(TOTAL_REVIEWS_FINDATTRS.name,
                                               TOTAL_REVIEWS_FINDATTRS.attrs).text.strip().replace(',', ''))

        self.review_page_json.link          = link
        self.review_page_json.maker         = maker
        self.review_page_json.average_stars = ave_star
    ...
    def _get_review_info(self, review_div) -> ReviewInfo:
        ...
        try:
            date   = self._extract_date(review_div.find(DATE_FINDATTRS.name, DATE_FINDATTRS.attrs).text.strip())
            star   = self._extract_stars(review_div.find(STAR_FINDATTRS.name, STAR_FINDATTRS.attrs).text.strip())
            vote   = self._extract_vote(review_div.find(VOTE_FINDATTRS.name, VOTE_FINDATTRS.attrs))
            name   = review_div.find(NAME_FINDATTRS.name, NAME_FINDATTRS.attrs).text.strip()
            title  = review_div.find(TITLE_FINDATTRS.name, TITLE_FINDATTRS.attrs).text.strip()
            review = review_div.find(REVIEW_FINDATTRS.name, REVIEW_FINDATTRS.attrs).text.strip()
